    ...
    def _execute_stage(self, stage_name: str, sql_file: str, dry_run: bool) -> bool:
        ...
        try:
            ...
            if not dry_run:
                ...
                logger.info(
                    f"[DRY RUN] Would execute {len([s for s in sql_content.split(';') if s.strip()])} statements"
                )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Validate pipeline without executing"
    )
    parser.add_argument("--export", type=str, help="Export forecast to CSV file at specified path")
    parser.add_argument(
        "--log-level",
        type=str,
        default="INFO",
