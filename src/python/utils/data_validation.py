...
class DataValidator:
    ...
    @staticmethod
    def validate_row_count(df: pd.DataFrame, min_rows: int = 0, max_rows: Optional[int] = None) -> bool:
        """
    ...
    def validate_column_values(df: pd.DataFrame, col: str, allowed_values: List[Any]) -> bool:
        ...
        if not invalid_values.empty:
            logger.warning(f"Column '{col}' contains invalid values: {invalid_values[col].unique()}")
            return False
...
class DataQualityReport:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        logger.info(f"Quality Report for {self.table_name}: {passed_checks}/{total_checks} checks passed")
        return report
